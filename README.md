...
| [ml-llm-alignment-fine-tuning](ml-llm-alignment-fine-tuning/) | Implements four LLM alignment techniques — supervised instruction fine-tuning, reward modeling, PPO-based RLHF, and Direct Preference Optimization — with LoRA on small open models (`opt-350m`, GPT-2), each evaluated with a metric suited to its objective (SacreBLEU, pairwise ranking accuracy, sentiment reward, preference accuracy). |
